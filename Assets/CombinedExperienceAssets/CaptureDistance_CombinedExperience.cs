...
public class CaptureDistance_CombinedExperience : MonoBehaviour
{
    ...
    public void beginNavigation() // Called either from Start (if just using Unity) or from UI button
    {
        ...
        {
            if ((GameObject.Find("arrow_CombinedExperience").GetComponent<moveTo>() == null) || (GameObject.Find("listOfFlagsGameObj_CombinedExperience").GetComponent<ListOps>() == null))
            {
            ...
            else // Otherwise, get first flag from the list and set it as destination
            {
                ...
                anchorsReceived++;
                GameObject.Find("arrow_CombinedExperience").GetComponent<moveTo>().setGoal(myGameObject.transform, myGameObject.name);
            }
        }
    }
    ...
    void Update()
    {
        ...
        if (MoveTo.goal) // Current goal from MoveTo
        {
            ...
            {
                //GameObject.Find("arrow_CombinedExperience").GetComponent<moveTo>().setGoal(ListOps.getNext().transform);
                GameObject updatedGoal = GameObject.Find("listOfFlagsGameObj_CombinedExperience").GetComponent<ListOps>().getNext();
                if (updatedGoal != null)
                {
                    GameObject.Find("arrow_CombinedExperience").GetComponent<moveTo>().setGoal(updatedGoal.transform, updatedGoal.name);
                    anchorsReceived++;
                }
                else
                {
                    ...
                    {
                        GameObject.Find("arrow_CombinedExperience").GetComponent<moveTo>().setGoal(null, "");
                        // Remove Arrow
                        Destroy(GameObject.Find("arrow_CombinedExperience"));
