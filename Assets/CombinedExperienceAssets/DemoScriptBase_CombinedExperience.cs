...
{
    public abstract class DemoScriptBase_CombinedExperience : InputInteractionBase
    {
        ...
        public async void AdvanceDemo()
        {
            ...
            {
                Debug.LogError($"{nameof(DemoScriptBase_CombinedExperience)} - Error in {nameof(AdvanceDemo)}: {ex.Message} {ex.StackTrace}");
        ...
        public async void EnumerateAllNearbyAnchors()
        {
            ...
            {
                Debug.LogError($"{nameof(DemoScriptBase_CombinedExperience)} - Error in {nameof(EnumerateAllNearbyAnchors)}: === {ex.GetType().Name} === {ex.ToString()} === {ex.Source} === {ex.Message} {ex.StackTrace}");
